...
const canvasOne =
  '<canvas id="canvasOne" width="800" height="500" class="graph"></canvas>';
table1.insertAdjacentHTML("beforebegin", canvasOne);

const canvasTwo =
  '<canvas id="canvasTwo" width="800" height="500" class="graph"></canvas>';
...
for (var i = 0; i < dataCells.length; i++) {
  // create for loop to iterate over array of dataCells
  if (i % 12 === 0) {
...
for (var country in data) {
  // create a for-in loop to iterate over the properties of the data object
  datasets.push({
...
var countries = [...table2.querySelectorAll("td")]
  .filter((td) => td.cellIndex === 1)
  .map((td) => td.innerText);
var thirdColumn = [...table2.querySelectorAll("td")]
  .filter((td) => td.cellIndex === 2)
  .map((td) => td.innerText);
var fourthColumn = [...table2.querySelectorAll("td")]
  .filter((td) => td.cellIndex === 3)
  .map((td) => td.innerText);
